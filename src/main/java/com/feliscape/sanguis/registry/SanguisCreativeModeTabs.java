...
public class SanguisCreativeModeTabs {
    private static final DeferredRegister<CreativeModeTab> REGISTER =
            DeferredRegister.create(Registries.CREATIVE_MODE_TAB, Sanguis.MOD_ID);
    ...
    private static class DisplayItemsGenerator implements CreativeModeTab.DisplayItemsGenerator {
        ...
        private List<Item> collectBlocks(Predicate<Item> exclusionPredicate) {
            ...
            for (DeferredHolder<Block, ?> entry : SanguisBlocks.BLOCKS.getEntries()) {
                Item item = entry.get()
                        .asItem();
                if (item == Items.AIR)
                    continue;
        ...
        private List<Item> collectItems(Predicate<Item> exclusionPredicate) {
            List<Item> items = new ReferenceArrayList<>();
            for (DeferredHolder<Item, ?> entry : SanguisItems.ITEMS.getEntries()) {
                Item item = entry.get();
